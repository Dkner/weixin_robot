...
class EditorRobot(object):
    ...
    def run(self, qr_callback):
        ...
        self.robot.run(debug=True, blockThread=False)
...
class FenqunRobot(EditorRobot):
    ...
    def __init__(self, id, robot):
        ...
        def add_friend(msg):
            ...
            :return:
            '''
            robot.add_friend(userName=msg['Text']['userName'], status=3)
            robot.send(FENQUN_AUTOREPLY, msg['RecommendInfo']['UserName'])
...
class ZongqunRobot(EditorRobot):
    def __init__(self, id, robot):
        ...
        def add_friend(msg):
            ...
            '''
            robot.add_friend(userName=msg['Text']['userName'], status=3)
            robot.send(ZONGQUN_AUTOREPLY, msg['RecommendInfo']['UserName'])
